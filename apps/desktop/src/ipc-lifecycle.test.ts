...
describe('desktop IPC shutdown gate', () => {
  ...
  it('bounds a stuck drain, prevents quit retries, and tears down each authority once', async () => {
    const order: string[] = [];
    const events: string[] = [];
    let quitCalls = 0;
    const shutdown = createDesktopShutdownCoordinator({
      credentials: { dispose: async () => { order.push('credentials'); } },
      lifecycle: { shutdown: async () => { order.push('lifecycle'); } },
      setup: { shutdown: async () => { order.push('setup'); } },
      operations: { shutdown: async cleanup => cleanup() },
      ipc: {
        close: () => { order.push('ipc-close'); },
        awaitIdle: () => new Promise<void>(() => undefined),
        dispose: () => { order.push('ipc-dispose'); },
      },
    ...
    assert.equal(events.filter(event => event === 'desktop.app.shutdown_retry').length, 1);
    assert.equal(events.filter(event => event === 'desktop.app.shutdown_forced').length, 1);
    for (const step of ['ipc-close', 'session-close', 'protocol-dispose', 'profiles-close',
      'session-dispose', 'ipc-dispose', 'window-destroy']) {
      assert.equal(order.filter(value => value === step).length, 1, `${step} was not exactly once`);
    }
    assert.equal(order.indexOf('profiles-close') > order.indexOf('protocol-dispose'), true);
    assert.deepEqual(order.slice(-4), ['profiles-close', 'session-dispose', 'ipc-dispose', 'window-destroy']);
  });

  for (const category of ['profile', 'pairing', 'session', 'setup'] as const) {
    it(`runs an admitted ${category} handler through the production before-quit drain`, async () => {
      ...
      const setup = {
        start: category === 'setup' ? begin : async () => ({ phase: 'idle' }),
        shutdown: async () => undefined,
      } as unknown as DesktopSetupController;
      const registered = registerIpcHandlers({
        app: {
          getName: () => 'ProPR', getVersion: () => '0.8.15', isPackaged: true,
        } as unknown as App,
      ...
      const shutdown = createDesktopShutdownCoordinator({
        ...
        setup: { shutdown: async () => { order.push('setup-shutdown'); } },
      ...
      else if (category === 'session') barrier.resolve(new Response(null, { status: 204 }));
      else barrier.resolve({ phase: 'cancelled' });
      await admitted;
      await shutdown.awaitFinished();
