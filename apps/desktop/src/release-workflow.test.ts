...
describe('desktop trusted release workflow', () => {
  ...
  test('exercises every staged Linux and macOS format through the native lifecycle gate', () => {
    ...
    assert.equal(validation.match(/test-native-artifact-lifecycle\.mjs/g)?.length, 2);
    ...
    for (const kind of ['deb', 'rpm', 'zip', 'dmg']) {
      assert.ok(nativeArtifactLifecycle.includes(`kind === '${kind}'`) || nativeArtifactLifecycle.includes(`kind !== '${kind}'`));
    }
    for (const linkEvidence of [
    ...
    ]) assert.ok(nativeArtifactLifecycle.includes(`desktop.deeplink.${linkEvidence}`));
