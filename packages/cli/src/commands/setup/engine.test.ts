...
test("direct webhooks + custom App auth offers manifest generation and writes it", async () => {
  let generateArgs: { rootDir: string; publicUrl: string; force?: boolean } | undefined;
  let inspectCalled = false;
  let promptCtx: { detectedPublicUrl?: string; filesExist: boolean } | undefined;
  const result = await runSetup({
    root: "/stack",
    prompts: {
      configureIntake: async () => ({ mode: "direct_webhook", webhookSecret: "s3cret" }),
      configureGithubAppManifest: async (ctx) => {
        promptCtx = { detectedPublicUrl: ctx.detectedPublicUrl, filesExist: ctx.filesExist };
        return { publicUrl: "https://propr.example.com" };
      },
    },
    actions: mockActions({
      detectGithubAuthMode: () => APP_AUTH,
      // .env carries a public URL, so the prompt should receive it pre-filled.
      ...
      generateGithubAppManifest: async (params) => {
        generateArgs = params;
        return {
          manifestPath: `${params.rootDir}/github-app-manifest.json`,
          envPath: `${params.rootDir}/github-app.env`,
          webhookUrl: "https://propr.example.com/webhook",
          createUrl: "https://github.com/settings/apps/new",
        };
      },
  ...
  assert.equal(promptCtx?.detectedPublicUrl, "https://propr.example.com", "public URL is detected from .env");
  assert.equal(promptCtx?.filesExist, false);
  assert.deepEqual(generateArgs, { rootDir: "/stack", publicUrl: "https://propr.example.com", force: undefined });
  // A generated manifest keeps the step done but surfaces the create/install steps.
  assert.equal(statusOf(result.state, "intake"), "done");
  assert.match(getStep(result.state, "intake")?.detail ?? "", /manifest written/i);
  assert.match(getStep(result.state, "intake")?.nextAction ?? "", /GH_APP_ID.*GH_INSTALLATION_ID.*HOST_GH_PRIVATE_KEY/);
});

test("manifest generation is not offered for polling or relay flows", async () => {
  let manifestPrompted = false;
  await runSetup({
    root: "/stack",
    prompts: {
      configureIntake: async () => ({ mode: "polling" }),
      configureGithubAppManifest: async () => {
        manifestPrompted = true;
        return null;
      },
    },
    actions: mockActions({ detectGithubAuthMode: () => APP_AUTH }),
  });

  assert.equal(manifestPrompted, false, "polling must not trigger manifest generation");
});

test("existing manifest files left as-is warn rather than fail when not regenerating", async () => {
  let generateCalled = false;
  const result = await runSetup({
    root: "/stack",
    prompts: {
      configureIntake: async () => ({ mode: "direct_webhook", webhookSecret: "s3cret" }),
      // The renderer declines to regenerate, returning null.
  ...
  assert.equal(generateCalled, false, "existing files must not be overwritten without regenerate");
...
test("a failed manifest generation degrades to a warning, not a setup failure", async () => {
  const result = await runSetup({
    ...
    prompts: {
      configureIntake: async () => ({ mode: "direct_webhook", webhookSecret: "s3cret" }),
      configureGithubAppManifest: async () => ({ publicUrl: "not-a-url" }),
    },
    actions: mockActions({
      detectGithubAuthMode: () => APP_AUTH,
      generateGithubAppManifest: async () => {
        throw new Error('Invalid --public-url: "not-a-url" is not a valid URL');
      },
    }),
  });

  assert.equal(statusOf(result.state, "intake"), "warning");
  assert.match(getStep(result.state, "intake")?.detail ?? "", /manifest not generated/i);
  assert.equal(result.completed, true);
});
