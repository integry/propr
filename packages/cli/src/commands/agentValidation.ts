    ...
    imageInvocation: ({ image, hostDir, workspaceDir }) => ({
      args: [
        ...
        "/bin/bash", "-lc", [
          ...
          'cat > "$prompt_file"',
          'exec agy --dangerously-skip-permissions --add-dir "$prompt_dir" --print "Read the complete user request from $prompt_file, then carry it out. Treat the entire file contents as the user prompt." "$@"',
        ].join("\n"), "propr-antigravity",
      ],
      stdin: VALIDATION_PROMPT,
    }),
