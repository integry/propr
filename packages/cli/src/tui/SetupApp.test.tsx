...
test("buildSetupPrompts confirms manifest generation and reuses a detected public URL", async () => {
  const bridge = new SetupBridge();
  const prompts = capture(bridge);
  const hooks = buildSetupPrompts(bridge);

  const decision = hooks.configureGithubAppManifest!({
    rootDir: "/stack",
  ...
  // First (and only) prompt: a generate? confirm. A known URL needs no input.
  assert.equal(prompts[0].kind, "confirm");
  bridge.resolve(prompts[0].id, true);
  assert.deepEqual(await decision, { publicUrl: "https://propr.example.com", regenerate: false });
});

test("buildSetupPrompts asks for a public URL when none is detected", async () => {
  const bridge = new SetupBridge();
  const seen: SetupPrompt[] = [];
  bridge.subscribe((event) => {
    if (event.type === "prompt") {
      ...
      queueMicrotask(() => {
        ...
        else if (prompt.kind === "input") bridge.resolve(prompt.id, "https://my.propr.example");
      });
    }
  });
  const hooks = buildSetupPrompts(bridge);
  const decision = await hooks.configureGithubAppManifest!({
    rootDir: "/stack",
  ...
  assert.ok(seen.some((p) => p.kind === "input"), "a public URL input is requested");
});

test("buildSetupPrompts leaves existing manifest files alone unless regenerate is confirmed", async () => {
  const bridge = new SetupBridge();
  const seen: SetupPrompt[] = [];
  bridge.subscribe((event) => {
    if (event.type === "prompt") {
      seen.push(event.prompt);
      // generate? → yes; regenerate existing files? → no.
      queueMicrotask(() => bridge.resolve(event.prompt.id, seen.length === 1));
    }
  });
  const hooks = buildSetupPrompts(bridge);
  const decision = await hooks.configureGithubAppManifest!({
  ...
  assert.equal(decision, null, "declining the regenerate confirm skips generation");
});
