...
type AuthPayloadData = Pick<SystemTaskJobData, 'type' | 'owner' | 'repoName' | 'prNumber' | 'userId' | 'requestingUser' | 'commitHash' | 'targetCommentId' | 'prBranch' | 'authTimestamp' | 'prHeadSha' | 'headRepoOwner' | 'headRepoName'>;
 ...
 */
export function buildAuthPayload(data: AuthPayloadData): string {
...
/** Build the payload used by jobs persisted before userId became mandatory. */
function buildLegacyAuthPayload(data: Omit<AuthPayloadData, 'userId'>): string {
 ...
 */
export function generateAuthToken(data: AuthPayloadData, secret: string): string {
...
export function verifyAuthToken(data: SystemTaskJobData, secret: string | undefined): { valid: boolean; reason?: string } {
    ...
    // BullMQ can deserialize jobs queued by the previous release without userId.
    // Only those jobs use the legacy format; current jobs remain bound to userId.
    const payload = Object.hasOwn(data, 'userId')
        ? buildAuthPayload(data)
        : buildLegacyAuthPayload(data);
    const hmac = crypto.createHmac('sha256', secret);
    hmac.update(payload);
