...
function updateMetadataFromEvent(event: CodexEvent, state: ParseState): void {
...
function handleItemCompletedEvent(event: CodexEvent, state: ParseState): void {
...
function processCodexEvent(event: CodexEvent, state: ParseState): void {
    updateMetadataFromEvent(event, state);

    switch (event.type) {
        case 'item.completed':
            handleItemCompletedEvent(event, state);
            break;
        case 'thread.started':
        ...
        case 'turn.completed':
            ...
            break;
        case 'item.started':
            // Skip - just a progress indicator
            break;
        default:
