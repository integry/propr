...
function handleItemCompleted(event: CodexEvent, state: ParseState): void {
    ...
    const itemHandlers: Record<string, () => void> = {
        'agent_message': () => {
    ...
    const handler = item.type ? itemHandlers[item.type] : undefined;
    if (handler) handler();
}
...
function processEvent(event: CodexEvent, state: ParseState): void {
    ...
    const eventHandlers: Record<string, () => void> = {
        'item.completed': () => handleItemCompleted(event, state),
        'thread.started': () => {
        ...
        'result': () => handleResultEvent(event, state),
        'turn.started': () => { state.logs += `[${event.type}]\n`; },
        'turn.completed': () => { state.logs += `[${event.type}]\n`; },
        'item.started': () => { /* Skip - just a progress indicator */ }
    };
