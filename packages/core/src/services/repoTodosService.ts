 ...
 */
export async function getCategoriesForRepository(
  userId: string,
  repository: string
...
export async function createCategory(params: CreateCategoryParams): Promise<RepoTodoCategory> {
  try {
    // If no orderIndex provided, put at the end
    let orderIndex = params.orderIndex;
    if (orderIndex === undefined) {
      const maxResult = await db<RepoTodoCategoryRecord>('repo_todo_categories')
        .where('user_id', params.userId)
        .andWhere('repository', params.repository)
        .max('order_index as maxOrder')
        .first() as { maxOrder: number | null } | undefined;
      orderIndex = ((maxResult?.maxOrder as number | null) ?? -1) + 1;
    }

    await db<RepoTodoCategoryRecord>('repo_todo_categories').insert({
    ...
    const record = await db<RepoTodoCategoryRecord>('repo_todo_categories')
...
export async function updateCategory(
  ...
  try {
    ...
    await db<RepoTodoCategoryRecord>('repo_todo_categories')
...
/**
 * Delete a category and optionally move its todos to uncategorized.
 */
export async function deleteCategory(categoryId: string, userId: string): Promise<boolean> {
  try {
    // Move all todos in this category to uncategorized
    await db<RepoTodoRecord>('repo_todos')
      .where('category_id', categoryId)
      .andWhere('user_id', userId)
      .update({
        category_id: null,
    ...
    const deleted = await db<RepoTodoCategoryRecord>('repo_todo_categories')
      .where('category_id', categoryId)
      .andWhere('user_id', userId)
      .del();
 ...
 */
export async function batchReorderCategories(
  userId: string,
  repository: string,
  ...
  try {
    await db.transaction(async (trx) => {
      for (const item of items) {
        await trx<RepoTodoCategoryRecord>('repo_todo_categories')
 ...
 */
export async function getTodosForRepository(
  userId: string,
  repository: string
): Promise<RepoTodo[]> {
  try {
...
export async function createTodo(params: CreateTodoParams): Promise<RepoTodo> {
  try {
    // If no orderIndex provided, put at the end of the category
    let orderIndex = params.orderIndex;
    if (orderIndex === undefined) {
      const query = db<RepoTodoRecord>('repo_todos')
        .where('user_id', params.userId)
        .andWhere('repository', params.repository);

      if (params.categoryId) {
        query.andWhere('category_id', params.categoryId);
      } else {
    ...
    await db<RepoTodoRecord>('repo_todos').insert({
...
export async function deleteTodo(todoId: string, userId: string): Promise<boolean> {
  try {
    const deleted = await db<RepoTodoRecord>('repo_todos')
      .where('todo_id', todoId)
      .andWhere('user_id', userId)
      .del();
 ...
 */
export async function batchReorderTodos(
  userId: string,
  repository: string,
  items: BatchReorderItem[]
): Promise<void> {
  try {
    await db.transaction(async (trx) => {
      for (const item of items) {
        ...
        // Allow moving to different category
        if (item.categoryId !== undefined) {
          updateData.category_id = item.categoryId;
        }

        await trx<RepoTodoRecord>('repo_todos')
 ...
 */
export async function linkTodosToDraft(
  todoIds: string[],
  draftId: string,
  userId: string
): Promise<number> {
  try {
    const updated = await db<RepoTodoRecord>('repo_todos')
...
/**
 * Mark all todos linked to a draft as completed.
 * Called when a draft/plan is merged.
 */
