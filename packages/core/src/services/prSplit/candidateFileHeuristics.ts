...
const SECRET_PATH = /(^|\/)(\.env(?:\..+)?|id_rsa|id_dsa|credentials?\.json|secrets?\.ya?ml)$|\.(pem|p12|pfx)$/i;
const SECRET_CONTENT = /-----BEGIN (?:RSA |EC |OPENSSH )?PRIVATE KEY-----|\bAKIA[0-9A-Z]{16}\b|\bgh[pousr]_[A-Za-z0-9]{30,}\b|\bgithub_pat_[A-Za-z0-9_]{30,}\b|\bxox[baprs]-[A-Za-z0-9-]{20,}\b/;
...
export function isSecretBearingSplitFile(file: PrSnapshotFile): boolean {
  ...
  return pathLooksSecret || SECRET_CONTENT.test(addedSplitPatchText(file));
}
