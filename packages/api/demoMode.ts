...
type RedisValueType = 'string' | 'list' | 'set';
...
export function createDemoRedisClient(): RedisClientType {
  ...
  const allKeys = (): string[] => Array.from(new Set([...strings.keys(), ...lists.keys(), ...sets.keys()]))
    .filter(key => !isExpired(key));
  const deleteKey = (key: string): boolean => {
    expirations.delete(key);
    keyTypes.delete(key);
    return strings.delete(key) || lists.delete(key) || sets.delete(key);
  };
