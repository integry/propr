...
export function buildMcpServer(principal: McpPrincipal, deps: ToolDeps, catalog: McpTool[]): McpServer {
  ...
  for (const [path, name] of [['connection', 'get_connection'], ['repositories', 'list_repositories'], ['models', 'list_models'], ['notifications', 'list_notifications'], ['activity', 'get_current_activity'], ['activity/recent', 'get_recent_activity']] as const) {
    server.registerResource(path.replace('/', '_'), `${prefix}/${path}`, { mimeType: 'application/json' }, async uri => ({ contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify(await call(name, {})) }] }));
  }
  ...
  server.registerResource('pull_requests', new ResourceTemplate(`${prefix}/repositories/{owner}/{repo}/pulls`, { list: undefined }), { mimeType: 'application/json' }, async (uri, vars) => ({ contents: [{ uri: uri.href, mimeType: 'application/json', text: JSON.stringify(await call('list_pull_requests', { repository: `${vars.owner}/${vars.repo}` })) }] }));
