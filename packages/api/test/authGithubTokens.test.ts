...
test('ensureAuthenticated rejects an expired GitHub token when refresh fails recoverably', async () => {
  configureDemoMode(false);
...
test('ensureAuthenticated rejects an expired GitHub token when refresh omits an access token', async () => {
  configureDemoMode(false);
  const req = createRequest(createUser());
  const { response, status } = createJsonResponse();
  globalThis.fetch = async () => new Response(JSON.stringify({
