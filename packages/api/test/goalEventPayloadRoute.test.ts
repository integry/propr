...
test('event route projects poisoned nested payloads without mutating persistence', async () => {
  ...
  const poisonedPayload = {
    ...
    xApiKey: 'short-one', 'X-API-Key': 'short-two', accessKey: 'short-access', auth_key: 'short-auth', 'signing-key': 'short-signing', nestedAccessKeyId: 'short-access-id',
  ...
  for (const forbiddenKey of [
    ...
    'controllerOwner', 'xApiKey', 'X-API-Key', 'accessKey', 'auth_key', 'signing-key', 'nestedAccessKeyId', 'leaseEpoch',
  ...
  for (const forbiddenLiteral of [
    ...
    'private-lease-owner', 'private-controller-owner', 'short-one', 'short-two', 'short-access', 'short-auth', 'short-signing', 'short-access-id',
