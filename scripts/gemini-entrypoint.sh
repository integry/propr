...
if [ -d "/home/node/.gemini" ]; then
    echo "Gemini config directory mounted"
    echo "Contents of /home/node/.gemini:"
    ls -la /home/node/.gemini/

    # Fix ownership if running with sudo capability
    if command -v sudo >/dev/null 2>&1; then
        echo "Fixing ownership of Gemini config files..."
        sudo chown -R node:node /home/node/.gemini 2>/dev/null || echo "Could not change ownership"
    fi

    # Ensure necessary subdirectories exist
    for dir in tmp; do
        if [ ! -d "/home/node/.gemini/$dir" ]; then
            echo "Creating missing directory: /home/node/.gemini/$dir"
            mkdir -p "/home/node/.gemini/$dir" 2>/dev/null || echo "Could not create $dir (permission issue)"
        fi
    done
else
    echo "WARNING: Gemini config directory not mounted at /home/node/.gemini"
fi

# Ensure Gemini config is accessible
if [ ! -f "/home/node/.gemini/oauth_creds.json" ]; then
    echo "Warning: Gemini OAuth credentials not found"
    echo "Ensure Gemini config directory is properly mounted"
    echo "Expected path: /home/node/.gemini/oauth_creds.json"
else
    echo "Gemini authentication configuration found"
fi
