...
{/* SCREENSHOT PLACEHOLDER (P1 — interim: the site's ui-planner.png until a fresh capture): Capture the Define & Context step with a filled prompt, an attached screenshot, the granularity selector on Balanced, the context level selector, and the cost preview visible. Use a real repository with Smart File Selection enabled. */}
...
{/* SCREENSHOT PLACEHOLDER (P2): Capture the Execution step for a plan with 3+ issues, showing per-issue agent/model selectors, the multi-model comparison toggle with Apply to All, and the Auto-merge, Epic PR, and Run ultrafix options. */}
