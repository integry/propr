...
Capture the Define & Context step with a filled prompt, an attached screenshot, the granularity selector on Balanced, the context level selector, and the cost preview visible. Use a real repository with Smart File Selection enabled.
:::
...
:::note Screenshot placeholder
Capture the Review Plan step showing a generated plan with several issues in the editor, the refinement chat panel, and the "Create N GitHub Issues" button.
:::
...
:::note Screenshot placeholder
Capture the Execution step for a plan with 3+ issues, showing per-issue agent/model selectors, the multi-model comparison toggle with Apply to All, and the Auto-merge, Epic PR, and Run ultrafix options.
:::
