...
- **`level-<level>`** overrides the configured reasoning level for that issue, for example `level-xhigh` or `level-max`. Valid levels are `low`, `medium`, `high`, `xhigh`, `max`, `ultra`, `ultracode`, and `auto`. Unlike model labels, adding a reasoning level does not fan out extra runs; if multiple reasoning labels are present, the highest-priority valid level applies to every run created for the issue.
