...
{/* SCREENSHOT PLACEHOLDER (P1 — one capture also serves operations/metrics.md's dashboard shot; interim: the site's ui-dashboard.png): Capture the dashboard home page with at least a few completed tasks, showing the summary strip, Happening now, and Recent outcomes. Run two or three small test issues first so the dashboard has real data. */}
