...
- **`level-<level>`** overrides the configured reasoning level for that issue, for example `level-xhigh` or `level-max`. Valid levels are `low`, `medium`, `high`, `xhigh`, `max`, `ultra`, `ultracode`, and `auto`. Unlike model labels, adding a reasoning level does not fan out extra runs; the first valid `level-*` label applies to every run created for the issue.
