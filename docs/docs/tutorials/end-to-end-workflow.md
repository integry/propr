...
This is one stage of ProPR, not a required pipeline. The issue you start from can be written by hand, produced by another planning tool, or generated by [Planner Studio](./planner-studio.md) — ProPR does not need to have planned it. Adding the processing label is the only thing required to start implementation, which makes "issue implementation only" a complete way to use ProPR.
