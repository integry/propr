...
Routing happens before the worker starts the runtime. Labels such as `llm-claude-fable51`, `llm-codex-astra`, `llm-opencode-big-pickle`, or dynamic slash-command selections resolve to a concrete agent and model. The worker then passes that model to the selected implementation, while the implementation translates ProPR model IDs into whatever the underlying CLI expects.
