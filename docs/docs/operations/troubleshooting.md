...
- **Browser notifications unavailable.** Automatic key setup runs at API startup. Check server output for storage errors, verify the existing instance data mount is writable, and restore corrupt `web-push/vapid.json` from backup (directory 700, file 600, service-account ownership). Repair and restart to retry; do not delete the identity as a routine fix. For explicit keys, set both `WEB_PUSH_VAPID_PUBLIC_KEY` and `WEB_PUSH_VAPID_PRIVATE_KEY` from one pair or remove both for automatic mode; the subject is independently optional. `propr check` validates explicit settings without printing keys. `WEB_PUSH_ENABLED=false` deliberately disables setup/enrollment/delivery. See [PWA, Web Push, and Badges](./pwa-web-push.md#configure-vapid) for backup/restore and rotation implications.
