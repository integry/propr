...
- **Something else uses host port 4000.** Irrelevant to the tunnel: Cloudflare forwards to the Docker-internal `http://api:4000`, not to the published host port, so the two cannot conflict.
