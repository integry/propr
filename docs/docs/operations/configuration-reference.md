...
| `GITHUB_REPOS_TO_MONITOR` | Unset | Optional authoritative, comma-separated repository list. When unset, the daemon uses repositories selected through setup or Settings and reloads that persisted list live. | Static environment-managed deployments only. |
