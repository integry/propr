...
| `PROPR_ADMIN_USERS` | Empty | Comma-separated authenticated GitHub usernames that bootstrap instance administrators. Administrators can manage installation settings, agents, access roles, and trusted agent runtime packages. The environment list remains authoritative as a break-glass mechanism; durable assignments are managed from **Web UI → Access** using stable GitHub user IDs. | Optional. |
| `PROPR_AGENT_RUNTIME_ADMIN_ANY_USER` | `false` | Deprecated compatibility opt-in that lets any authenticated ProPR user change installation-level agent runtime packages. **Warning:** runtime packages are installed as root into the trusted agent image, so enabling this grants every logged-in user root-equivalent control over the environment all agent jobs run in. Assign an administrator instead. | Optional. |
