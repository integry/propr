...
| `WEB_PUSH_VAPID_PRIVATE_KEY` | Unset | Secret half of the same P-256 VAPID pair used only for request signing. Its value is never returned by the API. | Web Push; keep server-side only. |
