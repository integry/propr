...
| `GITHUB_VISUAL_PREVIEW_TOKEN` | Unset | Advanced override for the OAuth token, classic PAT, or fine-grained PAT used only to upload visual-preview attachments. Normally an administrator login supplies an encrypted, automatically refreshed OAuth credential through **Settings → Visual preview uploads**. GitHub's user-attachment endpoint rejects App installation tokens. | Optional override. |
