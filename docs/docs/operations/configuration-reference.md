...
| `PROPR_ADMIN_USERS` | Empty | Comma-separated authenticated GitHub usernames that bootstrap instance administrators. Set at least one for initial setup, because unassigned users are members. The list remains authoritative while configured. **Username risk:** GitHub usernames can be renamed or recycled; store the bootstrap role from **Web UI → Access** to bind it to the numeric GitHub ID, then remove or carefully maintain the environment entry. | Initial setup, or optional break-glass access. |
