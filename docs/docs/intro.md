...
- **Label-Based Model Routing**: Route any issue to a specific agent and model with `llm-*` labels such as `llm-claude-sonnet46`, `llm-codex-gpt55`, or `llm-antigravity-pro-high`; several model labels on one issue produce a separate run, branch, and PR per model
