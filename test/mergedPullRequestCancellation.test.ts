...
describe('cancelMergedPullRequestTasks', () => {
  ...
  test('keeps merged-PR cancellation retryable while an abort-only stop remains active', async () => {
    const redisClient = createRedisClient();
  ...
  test('marks the PR merged gate before retrying an abort-only stop that remains active', async () => {
    const redisClient = {
      ...createRedisClient(),
