...
import { getAuthenticatedOctokit, generateCorrelationId, withErrorHandling, handleError, withRetry, retryConfigs, shutdownQueue, getDefaultModel, db, isEnabled as isDbEnabled, initializeWebhookHandler, handleCommentDeleted, handleCommentEdited, processCommentEvent } from '@gitfix/core';
import { logger } from '@gitfix/core';
import { pollForPullRequestComments } from './polling/prCommentPolling.js';
import { loadAllConfigs, reloadConfigs, getRepos, getBotUsername, getUserWhitelist, getPrimaryProcessingLabels, loadSettingsFromConfig } from './daemon/configLoader.js';
...
function getCommentConfig() {
    return {
        ...
        MODEL_LABEL_PATTERN,
        processCommentEvent: (payload, eventType, correlationId) => processCommentEvent(payload, eventType, correlationId, getCommentConfig())
    };
}
...
async function startDaemon(options = {}) {
    ...
    if (ENABLE_WEBHOOKS) {
        ...
        await initializeWebhookHandler((issue, correlationId) => processDetectedIssue(issue, correlationId, redisClient), (payload, eventType, correlationId) => processCommentEvent(payload, eventType, correlationId, commentConfig), (payload, eventType, correlationId) => handleCommentDeleted(payload, eventType, correlationId, commentConfig), (payload, eventType, correlationId) => handleCommentEdited(payload, eventType, correlationId, commentConfig));
...
const processCommentEventWrapper = (payload, eventType, correlationId) => processCommentEvent(payload, eventType, correlationId, getCommentConfig());
const handleCommentDeletedWrapper = (payload, eventType, correlationId) => handleCommentDeleted(payload, eventType, correlationId, getCommentConfig());
const handleCommentEditedWrapper = (payload, eventType, correlationId) => handleCommentEdited(payload, eventType, correlationId, getCommentConfig());
