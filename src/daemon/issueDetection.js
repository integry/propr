...
import { issueQueue } from '@gitfix/core';
...
export async function processDetectedIssue(issue, correlationId, redisClient) {
    ...
    const jobExists = existingJobs.some(job => job.name === 'processGitHubIssue' &&
    ...
    try {
        ...
        const addToQueueWithRetry = () => withRetry(() => issueQueue.add('processGitHubIssue', issueJob, {
            jobId,
            attempts: 3,
            backoff: { type: 'exponential', delay: 2000 },
        }), { ...retryConfigs.redis, correlationId }, `add_issue_to_queue_${issue.number}`);
...
export async function fetchIssuesForRepo(octokit, repoFullName, correlationId) {
    ...
    }, { ...retryConfigs.githubApi, correlationId }, `fetch_issues_${repoFullName}`);
    try {
