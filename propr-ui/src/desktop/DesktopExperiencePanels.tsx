...
import { AlertTriangle, ArrowLeft, ChevronRight, Cloud, Computer, LoaderCircle, Pencil, RefreshCw, Search, Server, Trash2 } from 'lucide-react';
...
export const ProfileEditor: React.FC<ProfileEditorProps> = ({ initial, candidate = false, notice, operationError, onCancel, onSave }) => {
  ...
  return (
    <form className="desktop-profile-form" onSubmit={submit}>
      ...
      <label>Display name<input autoFocus value={name} onChange={event => setName(event.target.value)} placeholder="Team ProPR" /></label>
      <label>Instance URL<input value={baseUrl} onChange={event => setBaseUrl(event.target.value)} inputMode="url" placeholder="https://propr.example.com" aria-describedby={error ? 'profile-url-error' : undefined} /></label>
...
export const InstanceChooser: React.FC<ChooserProps> = ({ profiles, busy, error, localSetupSupported, onLocalSetup, onConnectNew, onDiscover, ...listProps }) => (
...
export const ConnectionPanel: React.FC<{
  ...
  <main className="desktop-connection-card" aria-live="polite">
    ...
    {!result ? (
      ...
      <><div className={`desktop-connection-visual desktop-${result.status}`}><AlertTriangle /></div><span className="desktop-eyebrow">{connectionLabel(result)}</span><h1>{profile.name}</h1><p>{result.message || 'This instance needs authentication before ProPR Desktop can connect.'}</p>
