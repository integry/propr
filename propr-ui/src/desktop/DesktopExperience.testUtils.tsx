...
export const localProfile: DesktopProfile = {
  id: 'local', name: 'This computer', baseUrl: 'http://127.0.0.1:3000', kind: 'local',
};

export const remoteProfile: DesktopProfile = {
  id: 'remote', name: 'Team server', baseUrl: 'https://propr.example.com', kind: 'remote',
};
...
export const adaptersFor = (
  ...
  profiles: {
    list: vi.fn(async () => profiles), save: vi.fn(async () => undefined), remove: vi.fn(async () => undefined),
    getActiveId: vi.fn(async () => activeId), setActiveId: vi.fn(async () => undefined),
  },
  ...
  localSetup: {
    status: vi.fn(async () => ({
      ...
      sessionId: '00000000-0000-4000-8000-000000000000', rootDir: '/tmp/propr', logs: [],
    })),
    start: vi.fn(async () => ({
      phase: 'completed' as const,
      capability: { supported: true as const, kind: 'local' as const, platform: 'linux' as const },
      sessionId: '00000000-0000-4000-8000-000000000000', rootDir: '/tmp/propr', logs: [], profile: localProfile,
    })),
    retry: vi.fn(async () => { throw new Error('not used'); }),
    cancel: vi.fn(async () => ({
      phase: 'cancelled' as const,
      capability: { supported: true as const, kind: 'local' as const, platform: 'linux' as const },
      sessionId: '00000000-0000-4000-8000-000000000000', logs: [],
    })),
