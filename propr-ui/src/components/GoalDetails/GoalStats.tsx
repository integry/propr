...
export default function GoalStats({ stats }: { stats: GoalDetailStatsV1 }) {
  const items = [
    ...
    ['Time', `${formatDuration(stats.time.elapsedSeconds)} elapsed · ${formatDuration(stats.time.activeSeconds)} active · ${formatDuration(stats.time.pausedSeconds)} paused`],
    ['Message lag', `${stats.messages.queued} queued${stats.messages.oldestQueuedSeconds === null ? '' : ` · oldest ${formatDuration(stats.messages.oldestQueuedSeconds)}`}`],
    ['Passive artifacts', `${stats.artifacts.issues.total} issues · ${stats.artifacts.pullRequests.total} pull requests`],
  ];
  return (
    <section aria-labelledby="goal-stats-title" className="rounded-lg border border-slate-200 bg-white p-3">
      ...
      {stats.tokens.byProviderModel.length > 0 && (
