...
import { createGoalDetailHarness, cursorSequence, getGoalDetailMocks, detail, event, message, page, resetGoalDetailMocks, user } from './useGoalDetailTestHarness';
...
describe('useGoalDetail replay and authorization', () => {
  ...
  it('subscribes only after authorized history, uses exclusive reconnect cursors, deduplicates, and preserves older pagination', async () => {
    ...
    await waitFor(() => expect(mocks.socket.emit).toHaveBeenCalledWith('subscribe:goal', expect.objectContaining({ ownerId: 'owner-a', repository: 'integry/propr', goalId: 'goal-1', afterCursor: 'cursor:5' })));
    ...
    await waitFor(() => expect(mocks.getGoalEvents).toHaveBeenCalledWith('goal-1', expect.objectContaining({ afterCursor: 'cursor:6' })));
    ...
    await waitFor(() => expect(mocks.socket.emit).toHaveBeenCalledWith('subscribe:goal', expect.objectContaining({ afterCursor: 'cursor:6' })));
  });
  ...
  it('drains more than 200 replay events using each response cursor before reporting connected', async () => {
    ...
    mocks.getGoalEvents.mockImplementation((_goalId: string, options: { afterCursor?: string }) => {
      if (options.afterCursor === 'cursor:5') return Promise.resolve(page(Array.from({ length: 200 }, (_, index) => event(index + 6))));
      if (options.afterCursor === 'cursor:205') return Promise.resolve(page(Array.from({ length: 200 }, (_, index) => event(index + 206))));
    ...
    expect(mocks.getGoalEvents).toHaveBeenCalledWith('goal-1', expect.objectContaining({ afterCursor: 'cursor:205' }));
  ...
  it('bounds far-over-limit replay and live ingestion while recovering duplicates and a post-eviction gap', async () => {
    ...
    mocks.getGoalEvents.mockImplementation((_goalId: string, options: { afterCursor?: string }) => {
      const afterSequence = cursorSequence(options.afterCursor);
      if (afterSequence === undefined) return Promise.resolve(page([event(4), event(5)], true));
      if (afterSequence >= latestSequence) return Promise.resolve(page([]));
    ...
    mocks.getGoalEvents.mockImplementation((_goalId: string, options: { afterCursor?: string }) => {
      if (options.afterCursor === 'cursor:2405') return Promise.resolve(page([event(2_406), event(2_407)]));
      if (options.afterCursor !== undefined) return Promise.resolve(page([]));
      return Promise.resolve(page([event(4), event(5)], true));
    });
    act(() => deliver(2_407));
  ...
  it('coalesces authoritative detail reconciliation when an over-limit gap evicts its early detail events', async () => {
    ...
    const refreshed = {
      ...
      stats: { ...detail.stats, tokens: { total: 321, byProviderModel: [] } },
      latestSequence,
    };
    let gapOpened = false;
    ...
    mocks.getGoalEvents.mockImplementation((_goalId: string, options: { afterCursor?: string }) => {
      const afterSequence = cursorSequence(options.afterCursor);
      if (afterSequence === undefined) return Promise.resolve(page([event(4), event(5)], true));
      if (!gapOpened || afterSequence >= latestSequence) return Promise.resolve(page([]));
      const start = afterSequence + 1;
      const end = Math.min(latestSequence, start + 199);
      const events = Array.from({ length: end - start + 1 }, (_, index) => {
        const sequence = start + index;
        if (sequence === 6) return { ...event(sequence), eventType: 'lifecycle.state_changed' as const, kind: 'lifecycle' as const };
        if (sequence === 7) return { ...event(sequence), eventType: 'message.delivered' as const, kind: 'domain' as const };
        if (sequence === 8) return { ...event(sequence), eventType: 'usage.reported' as const, kind: 'domain' as const };
    ...
    expect((screen.getByTestId('event-types').textContent ?? '').split(',').every(type => type === 'provider.output')).toBe(true);

    const gapRequests = mocks.getGoalEvents.mock.calls.slice(replayRequestsBeforeGap)
      .filter(([, options]) => options.afterCursor !== undefined);
    expect(gapRequests.map(([, options]) => cursorSequence(options.afterCursor))).toEqual([5, 205, 405, 605, 805, 1_005]);
  ...
  it('bounds repeated older pages without losing a live tail delivered during the load', async () => {
    ...
    for (let repetition = 0; repetition < stressRepetitions; repetition += 1) {
      ...
      mocks.getGoalEvents.mockImplementation((_goalId: string, options: { afterCursor?: string; beforeCursor?: string }) => {
        if (options.afterCursor !== undefined) return Promise.resolve(page([]));
        const beforeSequence = cursorSequence(options.beforeCursor);
        if (beforeSequence === undefined) return Promise.resolve(page(Array.from({ length: 200 }, (_, index) => event(1_201 + index)), true, 1_201));
        if (beforeSequence === 1_201) return firstOlder.promise;
        const start = beforeSequence - 200;
      ...
      const olderCursors = mocks.getGoalEvents.mock.calls
        .map(([, options]) => cursorSequence(options.beforeCursor))
  ...
  it('immediately reconciles detail when initial history is newer than its snapshot', async () => {
    ...
    mocks.getGoalEvents.mockImplementation((_goalId: string, options: { afterCursor?: string }) => {
      if (options.afterCursor === 'cursor:6') return Promise.resolve(page([]));
      if (options.afterCursor !== undefined) return Promise.resolve(page([]));
      return Promise.resolve(page([event(4), event(5), { ...event(6), eventType: 'lifecycle.state_changed', kind: 'lifecycle' }]));
    });
    ...
    expect(mocks.getGoal).toHaveBeenCalledTimes(2);
    expect(mocks.getGoalEvents).toHaveBeenCalledWith('goal-1', expect.objectContaining({ afterCursor: 'cursor:6' }));
    expect(screen.getByTestId('events')).toHaveTextContent('4,5,6');
  ...
  it('retries replay reconciliation once against a mutation revision without committing stale detail', async () => {
    ...
    mocks.getGoalEvents.mockImplementation((_goalId: string, options: { afterCursor?: string }) => {
      if (options.afterCursor === 'cursor:5') return Promise.resolve(page([{ ...event(6), eventType: 'lifecycle.state_changed', kind: 'lifecycle' }]));
      if (options.afterCursor !== undefined) return Promise.resolve(page([]));
  ...
  it('keeps REST fallback active after connected-transport catch-up failure until replay recovers', async () => {
    ...
    mocks.getGoalEvents.mockImplementation((_goalId: string, options: { afterCursor?: string }) => {
      if (options.afterCursor === undefined) return Promise.resolve(page([event(4), event(5)], true));
      attempts += 1;
      if (attempts <= 3) return Promise.resolve({ ...page([]), nextCursor: 'cursor:5' });
