...
function CancelConfirmation({ busy, onClose, onConfirm }: { busy: boolean; onClose: () => void; onConfirm: (reason: string) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" role="presentation" onMouseDown={event => { if (event.target === event.currentTarget && !busy) onClose(); }}>
      <div role="alertdialog" aria-modal="true" aria-labelledby="cancel-goal-title" aria-describedby="cancel-goal-description" className="w-full max-w-md rounded-lg bg-white p-5 shadow-xl">
        ...
        <label className="mt-4 block text-sm font-medium text-slate-700">Cancellation reason<input value={reason} maxLength={1000} onChange={event => setReason(event.target.value)} className="mt-1 w-full rounded border border-slate-300 px-3 py-2 text-sm focus:border-red-500 focus:outline-none focus:ring-1 focus:ring-red-500" /></label>
        <div className="mt-5 flex justify-end gap-2"><button ref={cancelRef} type="button" onClick={onClose} disabled={busy} className="rounded border border-slate-300 px-3 py-2 text-sm">Keep running</button><button type="button" onClick={() => onConfirm(reason.trim())} disabled={busy || !reason.trim()} className="rounded bg-red-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-50">{busy ? 'Cancelling…' : 'Cancel goal'}</button></div>
      </div>
    </div>
  );
}
...
function LifecycleControls({ state, disabled, pendingAction, onPause, onResume, onOpenCancel }: {
  ...
  return (
    <div className="mt-3 flex flex-wrap gap-2">
      ...
      {!terminal && <button type="button" onClick={onOpenCancel} disabled={disabled} className="rounded border border-red-300 px-3 py-1.5 text-xs font-medium text-red-700 disabled:opacity-50">Cancel goal…</button>}
    </div>
  );
}
...
export default function GoalControls(props: GoalControlsProps) {
  ...
  return (
    <section aria-labelledby="goal-controls-title" className="rounded-lg border border-slate-200 bg-white p-3">
      ...
      <LifecycleControls state={detail.goal.state} disabled={controlsDisabled} pendingAction={pendingAction} onPause={onPause} onResume={onResume} onOpenCancel={() => setConfirmCancel(true)} />
      ...
      {confirmCancel && <CancelConfirmation busy={pendingAction === 'cancel'} onClose={() => setConfirmCancel(false)} onConfirm={reason => { void onCancel(reason).then(success => { if (success) setConfirmCancel(false); }); }} />}
    </section>
  );
}
