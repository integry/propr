...
describe('setupWizardHooks branch resolution', () => {
  ...
  it('continues generation when persisting the resolved baseBranch fails after draft creation', async () => {
    ...
    expect(navigate).toHaveBeenCalledWith('/studio/draft-1', expect.objectContaining({ replace: true, state: expect.objectContaining({ initialBaseBranch: 'develop', baseBranchPersistenceWarning: expect.stringContaining('failed to save base branch "develop"') }) }));
  ...
  it('persists only the explicitly selected repo-entry branch for repository restoration', () => {
    renderHook(() => usePlannerSettingsPersistence({ ...baseConfig, baseBranch: 'main' }, undefined, undefined, 'integry/propr', ''));
    expect(mockSavePlannerSettings).toHaveBeenCalledWith({ lastRepository: 'integry/propr', lastBaseBranch: null });
    expect(mockSavePlannerSettings).not.toHaveBeenCalledWith({ lastRepository: 'integry/propr', lastBaseBranch: 'main' });
  });
  it('persists the draft repository branch together with the draft repository in edit mode', () => {
    renderHook(() => usePlannerSettingsPersistence({ ...baseConfig, baseBranch: 'release' }, 'integry/propr', 'develop', 'integry/other', 'release'));
    expect(mockSavePlannerSettings).toHaveBeenCalledWith({ lastRepository: 'integry/propr', lastBaseBranch: 'develop' });
    expect(mockSavePlannerSettings).not.toHaveBeenCalledWith({ lastRepository: 'integry/propr', lastBaseBranch: 'release' });
  });
