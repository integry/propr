...
/** Dependency-free fallback for origins where secure-context SubtleCrypto is unavailable. */
const sha256Hex = (source: Uint8Array): string => {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temporary1 = (h + sum1 + choose + SHA_256_ROUND_CONSTANTS[index] + words[index]) >>> 0;
...
export const capacityFingerprint = async (account: CapacityFingerprintInput): Promise<string> => {
  ...
  const encodedSource = new TextEncoder().encode(fingerprintSource);
  try {
    const subtle = window.crypto?.subtle;
    if (subtle) {
      const digest = await subtle.digest('SHA-256', encodedSource);
    ...
    // Use the same deterministic fingerprint when Web Crypto exists but is
    // unusable, such as on a self-hosted non-secure origin.
  }
  return `sha256:${sha256Hex(encodedSource)}`;
};
