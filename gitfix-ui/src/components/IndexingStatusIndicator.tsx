...
const ReindexButton: React.FC<{ onReindex?: () => void }> = ({ onReindex }) => {
...
const getIndexingProgressInfo = (progress: RepositoryIndexingStatus['progress']) => {
  if (!progress) {
    return { progressText: 'Starting...', tooltipText: 'Indexing codebase...' };
  }
...
const IndexingStatus: React.FC<{ status: RepositoryIndexingStatus; onStop?: () => void }> = ({ status, onStop }) => {
  const progress = status.progress;
  const { progressText, tooltipText } = getIndexingProgressInfo(progress);
...
const CommitHashLink: React.FC<{ shortHash: string; fullHash: string | null; fullName: string | null }> = ({ shortHash, fullHash, fullName }) => {
  if (fullName) {
    return (
      <a
        href={`https://github.com/${fullName}/commit/${fullHash}`}
        target="_blank"
...
const CompletedStatus: React.FC<{ status: RepositoryIndexingStatus }> = ({ status }) => {
  ...
  return (
    <div className="flex items-center gap-1.5" title={fullTooltip}>
      ...
      {shortHash && (
        <CommitHashLink shortHash={shortHash} fullHash={status.last_indexed_hash} fullName={status.full_name} />
      )}
      {commitMessage && (
...
const renderStatusContent = (
  ...
  if (!status) return <NoInfoStatus />;

  switch (status.indexing_status) {
    case 'indexing':
      return <IndexingStatus status={status} onStop={onStop} />;
    case 'completed':
      return <CompletedStatus status={status} />;
    case 'failed':
      return <FailedStatus />;
    case 'idle':
    default:
...
export const IndexingStatusIndicator: React.FC<IndexingStatusIndicatorProps> = ({ status, onStop, onReindex }) => {
  return (
    <div className="flex items-center gap-3">
      {renderStatusContent(status, onStop)}
