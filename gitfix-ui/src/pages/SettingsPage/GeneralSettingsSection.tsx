...
const GeneralSettingsSection: React.FC<GeneralSettingsSectionProps> = ({
  ...
  // Helper to get pretty name
  const getModelLabel = (agentAlias: string, modelId: string) => {
    // Model ID in dropdown value might be namespaced or raw, 
    // but the model definition ID is what we want to match
  ...
  const renderModelOptions = () => {
    return agents.flatMap(agent => 
      agent.supportedModels.map(model => ({
        ...
        group: agent.enabled ? 'Enabled Agents' : 'Disabled Agents'
      }))
    );
  };

  const options = renderModelOptions();
  const enabledOptions = options.filter(o => o.enabled);
  const disabledOptions = options.filter(o => !o.enabled);
    ...
    <div className={`bg-white shadow rounded-lg p-6 ${className || ''}`}>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          >
            ...
            <optgroup label="Enabled Agents">
              {enabledOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
            </optgroup>
            {disabledOptions.length > 0 && (
              <optgroup label="Disabled Agents">
                {disabledOptions.map(opt => <option key={opt.value} value={opt.value} disabled>{opt.label}</option>)}
              </optgroup>
            )}
          </select>
          <p className="mt-1 text-sm text-gray-500">Used for initial triage and quick tasks.</p>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Used for deep-dive analysis and complex planning.</p>
        </div>
      </div>
    </div>
  );
